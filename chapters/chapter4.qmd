...
Now let's write some code. In `main.py`:
...
Four lines of actual code. Here's what each does.

`from fastapi import FastAPI` imports the FastAPI class.

`app = FastAPI()` creates your application. This object handles all the HTTP stuff.
...
A **route** is a path that your server responds to. An **endpoint** is the function that handles that route. Let's add more routes:
...
For a website that humans visit (not just an API for code to consume), we want to return HTML that browsers can render:
...
Copy your `profile.css` from Chapter 3 into the `static` folder. If you don't have it, here's a minimal version:
...
The `app.mount("/static", ...)` line tells FastAPI: any request starting with `/static/` should look for files in the `static` folder. This is how web applications serve CSS, JavaScript, and images.

The `profile` dictionary is our data. Right now it's hardcoded, but it could come from a database, a file, or user input.

The f-string builds the HTML dynamically. Change the name in the dictionary, save, and refresh. The page updates. This is the core idea of dynamic websites: the same code generates different pages based on different data.
